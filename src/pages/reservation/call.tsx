...
export default function Phone() {
  ...
  const { mutate: onReservate } = useMutation(
    ...
    {
      onSuccess: (data) => {
        queryClient.invalidateQueries([queryKey.예약하기.전화]);
      },
    }
  );
  ...
  const { mutate: onCancel } = useMutation(
    (id) => cancelReservate({ axiosInstance, category: "전화", id }),
    {
      onSuccess: (data) =>
        queryClient.invalidateQueries([queryKey.예약하기.전화]),
    }
  ...
  return (
    <>
      <ReservateWrapper title="전화" data={data} loading={isLoading}>
        <Input.Dummy
          placeholder=""
