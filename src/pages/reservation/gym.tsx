...
export default function Index() {
  ...
  const { mutate: onReservate } = useMutation(
    ...
    {
      onSuccess: (data) => {
        queryClient.invalidateQueries([queryKey.예약하기.운동]);
      },
    }
  );
  ...
  const { mutate: onCancel } = useMutation(
    (id) => cancelReservate({ axiosInstance, category: "운동", id }),
    {
      onSuccess: (data) =>
        queryClient.invalidateQueries([queryKey.예약하기.운동]),
    }
  ...
  return (
    <>
      <ReservateWrapper title="운동" data={data} loading={isLoading}>
        <Input.Dummy
          placeholder=""
