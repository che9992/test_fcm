...
export const Button = forwardRef<HTMLButtonElement, Props>(
  (props: Props, ref: ForwardedRef<HTMLButtonElement>) => {
    ...
    useEffect(() => {
      if (/Macintosh|iPhone|iPad|iPod|Android/.test(navigator.userAgent))
        setOs("mac");
    ...
    const TYPE_VARIANTS = {
      ...
      stroke: {
        ...
        border: BorderTheme({
          border: { solid: 1, position: "all", color: colors.keyColor },
        }),
      },
    };
    ...
    const borderT = border
      ? BorderTheme({ border })
      : (TYPE_VARIANTS[variant].border as any);
    const paddingT = padding
      ? SpaceTheme({ padding })
      : { padding: TAB_SIZE[as].padding };
    ...
    return (
      <button
        ...
        css={{
          ...
          backgroundColor:
            buttonColor ?? TYPE_VARIANTS[variant].backgroundColor,
